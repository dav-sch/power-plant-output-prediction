...
for (sc in c(1:4)){
     ...
     results_df_RMSE <- data.frame(matrix(ncol = 5, nrow = 0))
     col_names_RMSE <- c("RMSE/Capacity Linear Regression Train",
                    "RMSE/Capacity Naive Train",
                    "RMSE/Capacity Linear Regression Test",
                    "RMSE/Capacity Naive Test",
                    "Maximum output")
     colnames(results_df_RMSE) <- col_names_RMSE
     
     results_df_MAE <- data.frame(matrix(ncol = 5, nrow = 0))
     col_names_MAE <- c("MAE/Capacity Linear Regression Train",
                    "MAE/Capacity Naive Train",
                    "MAE/Capacity Linear Regression Test",
                    "MAE/Capacity Naive Test",
                    "Maximum output")
     ...
     for (ui in c(1:ncol(y_data_lag))){
          ...
          lin_reg <- lm(y_train_ ~ ., data=X_train_)
          
          # ET Classifier Results training set
          y_train_binary_predict <- y_predict_binary[-exclude_obs,ui]
          
          # Evaluate Regression results
          y_train_predict <- predict(lin_reg, X_train)
          y_train_predict[y_train_binary_predict==0] <- 0
          RMSE_linear_reg_train <- round(sqrt(mean((y_train_predict - y_train)^2))/max_temp, digits=3)
          MAE_linear_reg_train <- round(mean(abs(y_train_predict - y_train))/max_temp, digits=3)
          
          # NAIVE Predictor Average for all ones
          ...
          # ET Classifier Results test set
          y_test_binary_predict <- y_predict_binary[exclude_obs,ui]  
          
          # Evaluate Regression results
          y_test_predict <- predict(lin_reg, X_test)
          y_test_predict[y_test_binary_predict==0] <- 0
          RMSE_linear_reg <- round(sqrt(mean((y_test_predict - y_test)^2))/max_temp, digits=3)
          MAE_linear_reg <- round(mean(abs(y_test_predict - y_test))/max_temp, digits=3)
          ...
          row_temp <- data.frame(rbind(c(round(RMSE_linear_reg_train, digits=3),
                                         round(RMSE_naive_train, digits=3),
                                         round(RMSE_linear_reg, digits=3),
                                         round(RMSE_naive, digits=3),
                                         round(max_temp, digits=3))))
          rownames(row_temp) <- colnames(y_data_lag)[ui]
          colnames(row_temp) <- col_names_RMSE          
          print(row_temp$`RMSE/Capacity Linear Regression Train`)
          print(row_temp$`RMSE/Capacity Linear Regression Test`)

          results_df_RMSE <- rbind(results_df_RMSE, row_temp)
          
          row_temp <- data.frame(rbind(c(round(MAE_linear_reg_train, digits=3),
                                         round(MAE_naive_train, digits=3),
                                         round(MAE_linear_reg, digits=3),
                                         round(MAE_naive, digits=3),
                                         round(max_temp, digits=3))))
          rownames(row_temp) <- colnames(y_data_lag)[ui]
          colnames(row_temp) <- col_names_MAE         
          print(row_temp$`MAE/Capacity Linear Regression Train`)
          print(row_temp$`MAE/Capacity Linear Regression Test`)
          
          results_df_MAE <- rbind(results_df_MAE, row_temp)
     
     }
     
     saveRDS(results_df_RMSE, file=paste0("Linear_Regression_SAMPLE_",as.character(sc),"_data_v6_RMSE.rds"))
     saveRDS(results_df_MAE, file=paste0("Linear_Regression_SAMPLE_",as.character(sc),"_data_v6_MAE.rds"))
}
